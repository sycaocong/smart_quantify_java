...
public class KafkaConfig {
    ...
    public NewTopic orderExecutedTopic() {
        return TopicBuilder.name("order-executed")
                .partitions(3)
                .replicas(1)
                .build();
    }
    ...
    @Bean
    public ProducerFactory<String, OrderExecutedEvent> orderExecutedProducerFactory() {
        Map<String, Object> configProps = new HashMap<>();
        configProps.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapServers);
        configProps.put(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, StringSerializer.class);
        configProps.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, JsonSerializer.class);
    ...
    @Bean
    public KafkaTemplate<String, OrderExecutedEvent> orderExecutedKafkaTemplate() {
        return new KafkaTemplate<>(orderExecutedProducerFactory());
    }
}